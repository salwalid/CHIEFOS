#!/usr/bin/env python3
"""
Posts Hydrator
==============
Reads social_posts from SQLite → writes posts_data.json for the frontend.
No schema changes. Just reads what's there.
"""

import sqlite3
import json
import sys
from pathlib import Path

BASE_DIR = Path("$CHIEFOS_HOME")
DB_PATH = BASE_DIR / "chiefos.db"
OUTPUT_PATH = BASE_DIR / "www" / "HQ" / "posts" / "posts_data.json"


def hydrate():
    if not DB_PATH.exists():
        print(f"❌ Database not found: {DB_PATH}")
        sys.exit(1)

    conn = sqlite3.connect(str(DB_PATH))
    conn.row_factory = sqlite3.Row

    try:
        rows = conn.execute("SELECT * FROM social_posts ORDER BY id DESC").fetchall()
        posts = [dict(row) for row in rows]

        OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
        with open(OUTPUT_PATH, "w") as f:
            json.dump(posts, f, indent=2)

        total = len(posts)
        drafts = sum(1 for p in posts if p.get("status") == "Draft")
        published = sum(1 for p in posts if p.get("status") == "Published")
        print(f"✅ {total} posts hydrated ({drafts} draft, {published} published) → {OUTPUT_PATH}")

    except sqlite3.OperationalError as e:
        print(f"❌ DB error: {e}")
        sys.exit(1)
    finally:
        conn.close()


if __name__ == "__main__":
    hydrate()
